...
import be.ugent.verkeer4.verkeerrest.Routedata;
...
public class RoutedataFacadeREST extends AbstractFacade<Routedata> {
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") Integer id, Routedata entity) {
        super.edit(entity);
    }
    ...
    public Routedata find(@PathParam("id") Integer id) {
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Routedata> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
